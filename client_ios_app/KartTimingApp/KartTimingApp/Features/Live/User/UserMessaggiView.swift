...
// Icona condivisa dalla cronologia messaggi e dalla schermata team.
struct RacePenaltyIcon: View {
    let penalty: RacePenalty

    var body: some View {
        if penalty.systemIcon == "exclamationmark.triangle.fill" {
            Image(systemName: penalty.systemIcon)
                .symbolRenderingMode(.palette)
                .foregroundStyle(Color.black, Color.yellow)
        } else {
            Image(systemName: penalty.systemIcon)
                .foregroundColor(penalty.penaltyType == "blue_flag" ? .blue
                    : penalty.penaltyType == "black_flag" ? .red
                    : penalty.isWarning ? .kartDim : .kartWarningText)
        }
    }
}
